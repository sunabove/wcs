...
def main():
    ...
    try:
        while True:
            ...
            dt = 0.0 if prev_ts is None else (now_ts - prev_ts)
            prev_ts = now_ts
            ax, ay, az = imu.read_accel()
            gx, gy, gz = imu.read_gyro()
            acc_mag = (ax*ax + ay*ay + az*az) ** 0.5
            gyr_mag = (gx*gx + gy*gy + gz*gz) ** 0.5
            print(f"[{count:5d}] dt[s]={dt:6.3f} ACC[g] ({ax:7.3f}, {ay:7.3f}, {az:7.3f}), |ACC| {acc_mag:7.3f}")
            print(f"          GYR[°/s] ({gx:8.2f}, {gy:8.2f}, {gz:8.2f}), |GYR| {gyr_mag:8.2f}")
