...
window.latestVehicleOperationCommand =
  window.latestVehicleOperationCommand ?? null;
...
function mqttLog() {
  ...
  const topicLogPattern =
    /(wheel\/(fl|fr|rl|rr)\/(angle\/speed|linear\/speed|operation\/command|radius|angle)|vehicle\/operation\/command|vehicle\/linear\/speed)/i;
...
function renderObstacleFusionStatus() {
  ...
  const now = Date.now();
  const activeSensorIds = Array.from(
    obstacleFusionState.sensorLastSeenAtById.entries(),
  )
    .filter(
      ([, lastSeenAt]) =>
        Number.isFinite(lastSeenAt) &&
        now - lastSeenAt <= OBSTACLE_FUSION_SENSOR_RECEIVE_WINDOW_MS,
    )
  ...
  if ($confidence.length > 0) {
    $confidence.text(
      Number.isFinite(confidencePercent) ? `${confidencePercent}%` : "-%",
    );
  }
  ...
  if ($confidenceSlider.length > 0) {
    const sliderValue = Number.isFinite(confidencePercent)
      ? confidencePercent
      : 0;
    $confidenceSlider.val(sliderValue).css("background", "");
  }
}
...
function getConfiguredSensorCount(sensorId) {
  const configuredCount = Number.parseInt(
    sensorCountById[String(sensorId)],
    10,
  );
  ...
  const $chip = $(
    `#obstacle-sensor-types .obstacle-sensor-chip[data-sensor-id="${String(sensorId)}"]`,
  );
  if ($chip.length > 0) {
    const domCount = $chip.find(
      "[data-sensor-chip-number] .obstacle-sensor-chip-number-cell",
    ).length;
    if (domCount > 0) {
      return domCount;
    }
...
function refreshObstacleSensorChipNumbers(sensorId) {
  ...
  $(selector).each(function () {
    ...
    $(this)
      .find("[data-sensor-chip-number]")
      .html(getSensorCountRangeLabel(rowSensorId));
  });
}
...
function refreshSensorRowLabels(sensorId) {
  ...
  $(selector).each(function () {
    const rowSensorId = String($(this).attr("data-sensor-id") || "");
    const rowSensorIndex = Number.parseInt(
      $(this).attr("data-sensor-index"),
      10,
    );
    $(this)
      .find("[data-sensor-label]")
      .text(getSensorRowLabel(rowSensorId, rowSensorIndex));
    $(this)
      .find("[data-sensor-number]")
      .text(getSensorNumberLabel(rowSensorIndex));
  });
}
...
function applyObstacleSensorChipNumberState(sensorId) {
  ...
  $(selector).each(function () {
    ...
    $(this)
      .find("[data-sensor-chip-number] .obstacle-sensor-chip-number-cell")
      .each(function (cellIndex) {
        const dataIndex = Number.parseInt(
          $(this).attr("data-sensor-index"),
          10,
        );
        const normalizedCellIndex = Number.isFinite(dataIndex)
          ? dataIndex
          : cellIndex;
        const isActive = hasReceivedSensorIndex(
          rowSensorId,
          normalizedCellIndex,
        );
...
function updateReceivedSensorNumberCells(topic, value) {
  const sensorTopicMatch = String(topic || "").match(
    /^sensor\/([^/]+)\/(\d+)\/(state|value|obstacle|obstacle\/confidence)$/,
  );
  if (!sensorTopicMatch) {
    return;
  }
...
function updateObstacleFusionValues(topic, value) {
  ...
  if (topic === "obstacle/confidence") {
    ...
    obstacleFusionState.confidence = Number.isFinite(confidence)
      ? confidence
      : null;
    renderObstacleFusionStatus();
  }
...
function ensureDynamicSensorRow(topic) {
  ...
  $newRow
    .find("[data-sensor-label]")
    .text(getSensorRowLabel(safeSensorId, sensorIndex));
  ...
  rows.sort((a, b) => {
    ...
    const orderDelta =
      getSensorDisplayOrder(aSensorId) - getSensorDisplayOrder(bSensorId);
    if (orderDelta !== 0) {
      return orderDelta;
    }
...
function triggerObstacleSensorChipBlink(topic) {
  ...
  const $numberCell = $chip.find(
    `[data-sensor-chip-number] .obstacle-sensor-chip-number-cell[data-sensor-index="${sensorIndex}"]`,
  );
...
function dispatchVehicleDirectionEvent(sourceTopic, value) {
  ...
  try {
    window.dispatchEvent(
      new CustomEvent("wcs:vehicle-direction-update", {
        detail: {
...
function syncViewerDriveAnimationByCommand(commandValue) {
  ...
  if (typeof window.setDriveMode === "function") {
    const driveMode =
      typeof window.getVehicleDriveModeByCommand === "function"
...
function ensureFallbackAudioActivationListener() {
  if (
    fallbackVehicleAudioState.listenerAttached ||
    !readVehicleAudioEnabledFallback()
  ) {
    return;
  }
...
function processFallbackSpeechQueue() {
  ...
  if (
    !fallbackVehicleAudioState.isActivated ||
    fallbackVehicleAudioState.isSpeaking
  ) {
    return;
  }

  if (
    !Array.isArray(fallbackVehicleAudioState.speechQueue) ||
    fallbackVehicleAudioState.speechQueue.length === 0
  ) {
    return;
  }

  const nextMessage = String(
    fallbackVehicleAudioState.speechQueue.shift() || "",
  ).trim();
  if (!nextMessage) {
...
function speakVehicleStatusFallback(text, options = {}) {
  ...
  if (
    ...
    now - fallbackVehicleAudioState.lastSpokenAt <
      fallbackVehicleAudioState.duplicateMessageBlockMs
  ) {
    return;
  }
  ...
  if (shouldInterrupt && canUseSpeechSynthesisFallback()) {
    ...
    fallbackVehicleAudioState.speechQueue =
      fallbackVehicleAudioState.speechQueue.slice(-3);
  }
...
function announceVehicleRollAngleAudio(angleDeg) {
  ...
  if (fallbackVehicleAudioState.lastRollAngleDeg != null) {
    const angleDelta = Math.abs(
      roundedAngleDeg - fallbackVehicleAudioState.lastRollAngleDeg,
    );
    const elapsedMs = now - fallbackVehicleAudioState.lastRollAnnouncedAt;
    if (
      angleDelta < fallbackVehicleAudioState.minRollDeltaDeg ||
      elapsedMs < fallbackVehicleAudioState.minRollAnnounceIntervalMs
    ) {
      return;
    }
  }
  ...
  fallbackVehicleAudioState.lastRollAngleDeg = roundedAngleDeg;
  fallbackVehicleAudioState.lastRollAnnouncedAt = now;
  speakVehicleStatusFallback(`롤 각도 ${roundedAngleDeg}도`, {
    interrupt: true,
  });
}
...
function prcessMqttMessage(topic, value) {
  mqttLog(
    `[MQTT] 🧩 prcessMqttMessage 호출 - topic: ${topic}, value: ${value}`,
  );
  ...
  if (topic.startsWith("vehicle/")) {
    ...
    applyWheelAngleSpeedMessage(topic, value);
    ...
    if (match) {
      ...
      if (window.location.pathname.includes("110_vehicle_setting.html")) {
        // 현재 선택된 바퀴와 일치하는지 확인
        const currentSelectedWheel = $(
          'input[name="wheelPosition"]:checked',
        ).val();

        if (
          currentSelectedWheel &&
          currentSelectedWheel.toLowerCase() === wheelPosition.toLowerCase()
        ) {
          // 해당하는 wheelId 라디오 버튼 선택
          if (wheelId >= 1 && wheelId <= 4) {
            $(`input[name="wheelId"][value="${wheelId}"]`).prop(
              "checked",
              true,
            );
            mqttLog(
              `[MQTT] ✅ 바퀴 ID 자동 선택: ${wheelPosition.toUpperCase()} -> ID ${wheelId}`,
            );
  ...
  const sensorCountTopicMatch = String(topic || "").match(
    /^sensor\/([^/]+)\/count$/,
  );
  ...
  if (typeof window.updateRunInfoHistoryMetric === "function") {
    window.updateRunInfoHistoryMetric(topic, value);
  }
  if (typeof window.updateVehicleSpeedHistoryMetric === "function") {
    window.updateVehicleSpeedHistoryMetric(topic, value);
  }
  ...
  if (topic === "vehicle/surface/obstacle") {
    ...
    const $currentObstacleElement = $(
      `[id="vehicle/surface/obstacle/${obstacle}"]`,
    );
  ...
  if (
    topic === "vehicle/road/roll_angle" ||
    topic === "vehicle/road/pitch_angle"
  ) {
  ...
  if (topic === "vehicle/operation/command") {
    ...
    if (
      commandValue === 0 &&
      typeof window.clearVehicleWheelHighlights === "function"
    ) {
      window.clearVehicleWheelHighlights();
    } else if (
      commandValue === 1 &&
      typeof window.setVehicleWheelHighlightByKeys === "function"
    ) {
    ...
    // 해당 버튼 활성화
    const syncedDirectionUi =
      typeof window.syncVehicleDirectionButtons === "function"
        ? window.syncVehicleDirectionButtons(commandValue)
        : false;
    if (syncedDirectionUi) {
      mqttLog(
        `[MQTT] 🚗 차량 명령 버튼 선택: ${commandName} (${commandValue})`,
      );
    }
  }
  ...
  if (topic === "vehicle/linear/speed") {
    ...
    if (!shouldSkipAutoStopSync) {
      ...
      if (
        ...
        Math.abs(numericSpeed) <= speedZeroEpsilon
      ) {
        ...
        if (!vehicleSpeedZeroClickLatched && $stopButton.length > 0) {
          ...
          $stopButton.addClass("active text-white").removeClass("text-black");
        ...
        mqttLog(
          `[MQTT] ⏹️ 차량 속도 0 근접 감지(${numericSpeed.toFixed(3)}): 정지 버튼 UI 자동 동기화`,
        );
      } else if (
        Number.isFinite(numericSpeed) &&
        Math.abs(numericSpeed) >= speedReleaseEpsilon
      ) {
  ...
  if ($targetElement.length > 0) {
    ...
    const isSensorStateTopic = /^sensor\/[^/]+\/\d+\/state$/.test(
      String(topic || ""),
    );
...
$(document).ready(function () {
  ensureFallbackAudioActivationListener();
  if (typeof window.createRunInfoHistoryChart === "function") {
    window.createRunInfoHistoryChart();
  }
  if (typeof window.createVehicleSpeedHistoryChart === "function") {
    window.createVehicleSpeedHistoryChart();
  }
});
...
function applyDerivedWheelLinearSpeed(topic, value) {
  ...
  const formattedLinearSpeed = getFormattedTopicValue(
    linearSpeedTopic,
    linearSpeedMps,
  );
...
function applyWheelAngleSpeedMessage(topic, value) {
  if (topic !== "wheel/angle/speed") {
  ...
  wheelKeys.forEach(function (wheelKey, index) {
    const wheelTopic = `wheel/${wheelKey}/angle/speed`;
...
function applyWheelAngularVelocityToViewer(topic, value) {
  ...
  wheelSpeedLogCounter.byWheel[wheelKey] =
    (wheelSpeedLogCounter.byWheel[wheelKey] || 0) + 1;
...
function getFormattedTopicValue(topic, value) {
  ...
  if (topic.startsWith("sensor/") && topic.endsWith("/state")) {
    ...
    const confidencePercent = Number.isFinite(numValue)
...
function updateTargetElementCss($targetElement) {
  ...
  if (
    $parentRow.length > 0 &&
    $parentRow.hasClass("sensor-info-row-inactive")
  ) {
    return;
  }
  ...
  if (!colorPair) {
    console.warn(
      "[CSS] 색상 배열에서 유효한 colorPair를 찾을 수 없음. rowIndex:",
      rowIndex,
    );
