(function () {
  ...
  const OVERLAY_PAUSED_FRAME_POSITION_STORAGE_KEY =
    "wcs.status.overlay.paused_frame_position";
  ...
  const showVideoOverlayEnabled =
    $viewer.length > 0 && toBoolean($viewer.attr("showVideo"));
  const showWheelInfoOverlayEnabled =
    $viewer.length > 0 && toBoolean($viewer.attr("showWheelInfo"));
  ...
  function setCloseButtonToShowMode(_isShowMode) {
    $closeButton
      ...
      .html(
        '<i class="bi bi-x-lg overlay-toggle-icon overlay-toggle-icon-close" aria-hidden="true"></i>',
      );
  }
  ...
  function readOverlayMediaHiddenState() {
    try {
      const rawValue = window.localStorage.getItem(
        OVERLAY_MEDIA_HIDDEN_STORAGE_KEY,
      );
  ...
  function readCurrentVideoSelectionState() {
    try {
      return String(
        window.localStorage.getItem(CURRENT_VIDEO_SELECTION_STORAGE_KEY) || "",
      ).trim();
    } catch (error) {
      return "";
    }
  ...
  function readOverlayPausedFramePosition(videoFileName) {
    try {
      const rawValue = window.sessionStorage.getItem(
        OVERLAY_PAUSED_FRAME_POSITION_STORAGE_KEY,
      );
      const savedPosition = rawValue ? JSON.parse(rawValue) : null;
      const frameNumber = Number(savedPosition?.frameNumber);
      if (
        !savedPosition ||
        normalizePath(savedPosition.fileName) !==
          normalizePath(videoFileName) ||
  ...
  function writeOverlayPausedFramePosition() {
    if (!latestCurrentVideoFileName || roadFileOverlayFrameNumber < 1) {
      return;
    }
    ...
    try {
      window.sessionStorage.setItem(
        ...
        JSON.stringify({
          fileName: latestCurrentVideoFileName,
          frameNumber: roadFileOverlayFrameNumber,
        }),
      );
    } catch (error) {
  ...
  function syncVehicleTransparencyControl() {
    const vehicleViewer = window.urdfViewersById?.["vehicle-urdf-viewer"];
    if (
      vehicleViewer &&
      typeof vehicleViewer.setCarFrameOpacityControlVisible === "function"
    ) {
      vehicleViewer.setCarFrameOpacityControlVisible(mediaHiddenByUser);
    }
  }
  ...
  function updateOverlayAudioHud() {
    ...
    const isRecentSpeech =
      Number.isFinite(lastSpeechAt) && Date.now() - lastSpeechAt < 900;
    const isSpeakingNow =
      audioEnabled &&
    ...
    $audioHudText.text(
      audioEnabled ? speechText : "새로고침 후 이곳을 한번 클릭하세요",
    );
    $audioHudState.css(
      "background",
      audioEnabled ? "rgba(25, 135, 84, 0.70)" : "rgba(108, 117, 125, 0.72)",
    );
  ...
  function showTemporaryStatusMessage(
    message,
    durationMs = TEMPORARY_STATUS_MESSAGE_MS,
  ) {
  ...
  function getOrCreateOverlayToastContainer() {
    let container = document.getElementById(
      "road-detect-overlay-toast-container",
    );
    if (container) {
      return container;
    }
  ...
  const resolveRoadDetectStreamPath =
    typeof window.wcsResolveRoadDetectStreamPath === "function"
  ...
  const buildRoadDetectStreamUrl =
    typeof window.wcsBuildRoadDetectStreamUrl === "function"
      ? function (fileName) {
          return window.wcsBuildRoadDetectStreamUrl(
            fileName,
            buildRoadDetectQueryOptions({ show_time_bar: true }),
          );
        }
      : function (fileName) {
  ...
  const buildRoadDetectStreamCleanupUrl =
    typeof window.wcsBuildRoadDetectStreamCleanupUrl === "function"
      ? function (fileName) {
      ...
      : function (fileName) {
          ...
          return (
            "/fast/road_detect_stream_cleanup/" +
            encodedPath +
            "?" +
            $.param({
              t: Date.now(),
  ...
  function buildRoadDetectStreamSeekUrl(sessionId, frameNumber) {
    return (
      "/fast/road_detect_stream_seek/" +
      encodePathForRoute(sessionId) +
      "?" +
      $.param({ frame_number: frameNumber })
    );
  }
  ...
  function startRoadFileOverlayStream(fileName) {
    ...
    const restoreFrameNumber = shouldRestorePausedState
      ? readOverlayPausedFramePosition(fileName)
      : 0;
    ...
    roadFileOverlayInitRequest = $.ajax({
      ...
      .done(function (result) {
        ...
        if (restoreFrameNumber > 0) {
          $.ajax({
            url: buildRoadDetectStreamSeekUrl(sessionId, restoreFrameNumber),
            method: "POST",
            timeout: 5000,
          }).always(function () {
  ...
  function buildCameraDetectStreamInitUrl(cameraIndex) {
    return (
      "/fast/camera_detect_stream_init?" +
      $.param(
        buildRoadDetectQueryOptions({
  ...
  function buildCameraDetectStreamNextUrl(sessionId) {
    return (
      "/fast/camera_detect_stream_next/" +
  ...
  function buildCameraDetectStreamCleanupUrl(sessionId) {
    return (
      "/fast/camera_detect_stream_cleanup/" +
      encodeURIComponent(String(sessionId || ""))
    );
  }
  ...
  function stopCameraOverlayStream(sendCleanup = true) {
    ...
    if (
      cameraOverlayNextRequest &&
      typeof cameraOverlayNextRequest.abort === "function"
    ) {
    ...
    if (
      cameraOverlayInitRequest &&
      typeof cameraOverlayInitRequest.abort === "function"
    ) {
    ...
    if (
      cameraOverlayCleanupRequest &&
      typeof cameraOverlayCleanupRequest.abort === "function"
    ) {
  ...
  function scheduleCameraOverlayNextPoll(delayMs) {
    clearCameraOverlayPoll();
    cameraOverlayPollTimerId = setTimeout(
      function () {
    ...
    cameraOverlayNextRequest = $.ajax({
      ...
      .done(function (result) {
        ...
        const frameB64 = String(
          result.frame_detected || result.frame_original || "",
        ).trim();
        if (!frameB64) {
  ...
  function buildRoadDetectStreamCleanupUrlByPath(filePath) {
    ...
    return (
      "/fast/road_detect_stream_cleanup/" +
      encodePathForRoute(normalizedPath) +
      "?" +
      $.param({
        t: Date.now(),
      })
  ...
  function extractRoadDetectStreamFilePathFromUrl(url) {
    ...
    const rawPath =
      queryIndex >= 0
        ? text.slice(startIndex, queryIndex)
  ...
  function requestRoadDetectSessionCleanup(fileName) {
    ...
    const byFileNameUrl = buildRoadDetectStreamCleanupUrl(
      fileName || latestCurrentVideoFileName,
    );
    if (byFileNameUrl) {
    ...
    const streamedFilePath =
      extractRoadDetectStreamFilePathFromUrl(lastMediaSource);
    const byStreamPathUrl =
      buildRoadDetectStreamCleanupUrlByPath(streamedFilePath);
    if (byStreamPathUrl) {
  ...
  function requestCleanupOnPageExit() {
    ...
    const roadCleanupUrl =
      "/fast/road_detect_stream_cleanup_all?" + $.param({ t: Date.now() });
  ...
  function attemptImageStreamAutoReplay() {
    ...
    if (parseCameraSelection(latestCurrentVideoFileName)) {
      ...
      if (
        nowForCamera - lastImageReplayAttemptAt <
        IMAGE_STREAM_REPLAY_COOLDOWN_MS
      ) {
        return;
      }
    ...
    if (
      lastImageFrameAt <= 0 ||
      now - lastImageFrameAt < IMAGE_STREAM_STALE_MS
    ) {
      return;
    }

    const statusText = String($status.text() || "").trim();
    const isTimeoutState =
      imageStreamNeedsReplay ||
      (!$status.hasClass("d-none") &&
        statusText === FIRST_FRAME_TIMEOUT_MESSAGE);
    if (!isTimeoutState) {
      return;
    }
  ...
  function applyVehicleViewerDragByOverlayHeight() {
    ...
    window.setVehicleViewerOverlayDragPixels(
      overlayHeightPx * VIEWER_DRAG_PIXELS_RATIO,
    );
  }
  ...
  function getCompactOverlayMaxWidth() {
    ...
    const reservedWidth =
      estimatedWheelPanelWidth * 2 + sideMargin * 2 + centerGap;
  ...
  function applyCollapsedOverlayLayout() {
    ...
    $overlay.attr(
      ...
      "inset:auto;top:10px;left:50%;transform:translateX(-50%);width:" +
        collapsedWidth +
        ";height:52px;z-index:60;display:block;background:transparent;border-radius:0;overflow:visible;",
    );
  ...
  function resolveAndShowCurrentVideo(fileName) {
    ...
    const isCurrentVideoVisible =
      $video.length > 0 && !$video.hasClass("d-none");
    const isSameVideoSelection =
      lastMediaType === "video" &&
    ...
    if (
      isSameVideoSelection &&
      hasCurrentVideoSource &&
  ...
  $playToggleButton.on("click", function () {
    ...
    const videoElement = $video[0];
    const hasImageMediaSource =
      !!String(lastMediaSource || "").trim() || isCameraSelectionActive;
    ...
    if (lastMediaType === "image" && hasImageMediaSource) {
      if (mediaPlaybackPaused) {
        ...
        if (isCameraSelectionActive && cameraSelection) {
          ...
          const cacheBustSeparator = lastMediaSource.includes("?") ? "&" : "?";
          showImageSource(
            `${lastMediaSource}${cacheBustSeparator}t=${Date.now()}`,
          );
        }
      } else {
        mediaPlaybackPaused = true;
  ...
  window.addEventListener("urdfviewerready", syncVehicleTransparencyControl);

  window.addEventListener("pagehide", requestCleanupOnPageExit, {
    capture: true,
  });
  window.addEventListener("beforeunload", requestCleanupOnPageExit, {
    capture: true,
  });
  ...
  if (
    showVideoOverlayEnabled &&
    $overlay.hasClass("d-none") &&
    !hasStoredOverlayMediaHiddenState
  ) {
    mediaHiddenByUser = true;
    writeOverlayMediaHiddenState(true);
    setCloseButtonToShowMode(true);
