...
class MqttClientManager {
  ...
  constructor(options = {}) {
    ...
    this.clientId =
      options.clientId ||
      `vehicle_status_client_${Math.random().toString(16).substr(2, 8)}`;
  ...
  static getConsoleLogEnabled() {
    try {
      const saved = window.localStorage.getItem(
        MqttClientManager.logOptionStorageKey,
      );
      if (saved === null) {
        return false;
      }
  ...
  bindTopicHistoryHandlers() {
    ...
    $(document)
      ...
      .on("click.mqttTopicHistory", selector, (event) => {
        const historyType = String(
          $(event.currentTarget).attr("data-history-type") || "",
        ).toLowerCase();
    ...
    $(document)
      ...
      .on("click.mqttTopicSort", sortSelector, (event) => {
        const sortKey = String(
          $(event.currentTarget).attr("data-sort-key") || "",
        ).toLowerCase();
    ...
    $(document)
      ...
      .on("input.mqttTopicFilter", filterSelector, (event) => {
        ...
        const historyType = this.normalizeHistoryType(
          $input.attr("data-history-type"),
        );
  ...
  updateTopicFilterUi() {
    ...
    $("#mqtt-topic-filter-received").toggleClass(
      "d-none",
      currentType !== "received",
    );
    $("#mqtt-topic-filter-published").toggleClass(
      "d-none",
      currentType !== "published",
    );
  }
  ...
  toggleTopicHistorySort(sortKey) {
    ...
    if (this.topicHistorySort.key === sortKey) {
      this.topicHistorySort.direction =
        this.topicHistorySort.direction === "asc" ? "desc" : "asc";
    } else {
    ...
    const currentList =
      this.currentHistoryType === "published"
        ? this.publishedTopicHistory
        : this.receivedTopicHistory;
    const title =
      this.currentHistoryType === "published"
  ...
  getSortedTopicHistory(historyList) {
    ...
    sortedHistory.sort((a, b) => {
      ...
      const leftTime =
        a.time instanceof Date ? a.time.getTime() : Number.MIN_SAFE_INTEGER;
      const rightTime =
        b.time instanceof Date ? b.time.getTime() : Number.MIN_SAFE_INTEGER;
  ...
  renderTopicHistoryRows(historyList, title) {
    ...
    const filteredHistoryList = this.getFilteredTopicHistory(
      historyList,
    ...
    sortedHistoryList.forEach((entry, index) => {
      ...
      const shouldBlink =
        Number.isFinite(Number(entry.highlightUntil)) &&
        Number(entry.highlightUntil) > now;
      const $row = $("<tr></tr>");
      if (
        shouldBlink &&
        (this.currentHistoryType === "received" ||
        ...
        $row.addClass("mqtt-topic-history-row-live-blink");
      }
      $('<td class="small text-end text-nowrap"></td>')
        .text(String(index + 1))
        .appendTo($row);
      $('<td class="small text-nowrap text-center"></td>')
        .text(formattedTime)
        .appendTo($row);
      $('<td class="small"></td>')
        .text(String(entry.topic || ""))
        .appendTo($row);
  ...
  bindTopicHistoryDragHandlers() {
    ...
    const cancelSelector =
      "button, input, textarea, select, a, .btn-close, .mqtt-topic-sort-trigger, .mqtt-topic-history-tab, .mqtt-topic-filter-input";
    ...
    $(document)
      ...
      .on("mousedown.mqttTopicHistoryDrag", handleSelector, (event) => {
        ...
        const dialogElement = modalElement
          ? modalElement.querySelector(".mqtt-topic-history-dialog")
          : null;
    ...
    $(document)
      ...
      .on("mousemove.mqttTopicHistoryDrag", (event) => {
        ...
        const nextLeft = Math.min(
          maxLeft,
          Math.max(8, this.topicHistoryDragState.originLeft + dx),
        );
        const nextTop = Math.min(
          maxTop,
          Math.max(8, this.topicHistoryDragState.originTop + dy),
        );
  ...
  clampTopicHistoryDialogInViewport() {
    ...
    const clampedLeft = Math.min(
      maxLeft,
      Math.max(8, Number.isFinite(currentLeft) ? currentLeft : rect.left),
    );
    const clampedTop = Math.min(
      maxTop,
      Math.max(8, Number.isFinite(currentTop) ? currentTop : rect.top),
    );
  ...
  handleMessage(topic, message) {
    ...
    if (topic.startsWith("vehicle/") || topic.startsWith("wheel/")) {
      ...
      MqttClientManager.log(
        `[MQTT] 📝 ${topic.split("/")[0]}/*: ${messageStr}`,
      );
    }
    ...
    if (shouldCountAsReceived) {
      ...
      this.triggerCounterBadgeBlink(
        "#mqtt-received-badge",
        "mqtt-counter-badge-blink-received",
      );
    }
  ...
  parseValue(messageStr) {
    const trimmedValue = String(messageStr).trim();
    if (trimmedValue === "") {
      return messageStr;
    }

    const numValue = Number(trimmedValue);
    return Number.isFinite(numValue) ? numValue : messageStr;
  }
  ...
  publishClientConnectInfo() {
    ...
    this.client.publish("client/connect", JSON.stringify(clientInfo), {
      qos: 1,
    });
    ...
    this.client.publish(topic, messageStr, { qos: qosValue }, (err) => {
      ...
      if (!err) {
        ...
        this.triggerCounterBadgeBlink(
          ...
          "mqtt-counter-badge-blink-published",
        );
        $("#mqtt-published-badge")
          .removeClass("bg-info bg-success bg-warning bg-primary bg-secondary")
          .addClass("bg-primary");
        this.updateTopicCounters();
        MqttClientManager.log(
          `[MQTT] 📤 [${timestamp}] 메시지 전송성공 [QoS ${qosValue}]:`,
          topic,
          messageStr,
        );
        return;
  ...
  publishDisconnectInfo() {
    ...
    try {
      ...
      const disconnectInfo = {
        ...
        session_duration:
          now.getTime() - (window.clientConnectTime || now.getTime()),
      };
...
window.WcsMqtt = {
  ...
  sendMQTTMessage: (topic, message, qos) =>
    MqttClientManager.send(topic, message, qos),
  getConsoleLogEnabled: () => MqttClientManager.getConsoleLogEnabled(),
  setConsoleLogEnabled: (enabled) =>
    MqttClientManager.setConsoleLogEnabled(enabled),
