(function () {
  ...
  const uploadedLoadingElement = document.getElementById(
    "sam2-uploaded-loading",
  );
  const uploadedLoadingTextElement = document.getElementById(
    "sam2-uploaded-loading-text",
  );
  ...
  const uploadProgressWrapElement = document.getElementById(
    "sam2-upload-progress-wrap",
  );
  const uploadProgressBarElement = document.getElementById(
    "sam2-upload-progress-bar",
  );
  const uploadProgressStatusElement = document.getElementById(
    "sam2-upload-progress-status",
  );
  const promptFrameInput = document.getElementById("sam2-prompt-frame-input");
  const foregroundPointModeButton = document.getElementById(
    "sam2-foreground-point-mode",
  );
  const backgroundPointModeButton = document.getElementById(
    "sam2-background-point-mode",
  );
  ...
  const pointToolbarTitle = document.querySelector(
    "#sam2-input-pane .sam2-point-toolbar > div > .fw-semibold",
  );
  pointToolbarTitle?.classList.add("d-none");
  const bboxModeButton = document.getElementById("sam2-bbox-mode");
  const foregroundPointClearButton = document.getElementById(
    "sam2-foreground-point-clear",
  );
  const backgroundPointClearButton = document.getElementById(
    "sam2-background-point-clear",
  );
  ...
  if (optionsResetButton) {
    ...
    optionsResetButton.title =
      "현재 변경사항을 취소하고 저장된 검출 설정으로 복원";
  }
  const multimaskOutputCheckbox = document.getElementById(
    "sam2-multimask-output",
  );
  ...
  const referenceScoreInput = document.getElementById(
    "sam2-reference-score-input",
  );
  const maskRatioInput = document.getElementById("sam2-mask-ratio-input");
  ...
  if (maskRatioInput) {
    ...
    maskRatioInput.addEventListener("blur", () => {
      maskRatioInput.value = formatMaskRatioInputValue(maskRatioInput.value);
    });
    maskRatioInput.addEventListener("change", () => {
      maskRatioInput.value = formatMaskRatioInputValue(maskRatioInput.value);
    });
  }
  document
    .getElementById("sam2-point-label-text")
    ?.closest(".form-switch")
    ?.remove();
  const multimaskOutputText = document.getElementById(
    "sam2-multimask-output-text",
  );
  const maskInputText = document.getElementById("sam2-mask-input-text");
  const positivePointListElement = document.getElementById(
    "sam2-positive-points",
  );
  const positivePointCountElement = document.getElementById(
    "sam2-positive-count",
  );
  const pointMarkerLayerElement = document.getElementById(
    "sam2-point-marker-layer",
  );
  const bboxLayerElement = document.getElementById("sam2-bbox-layer");
  const bboxCaptureLayerElement = document.getElementById(
    "sam2-bbox-capture-layer",
  );

  const statusElement = document.getElementById("sam2-status");
  const inputVideoElement = document.getElementById("sam2-input-video");
  const inputFrameCounterElement = document.getElementById(
    "sam2-input-frame-counter",
  );
  const outputVideoElement = document.getElementById("sam2-output-video");
  const outputPlayToggleButton = document.getElementById(
    "sam2-output-play-toggle",
  );
  const outputFrameSliderElement = document.getElementById(
    "sam2-output-frame-slider",
  );
  const outputFrameSpinnerElement = document.getElementById(
    "sam2-output-frame-spinner",
  );
  ...
  const yoloDatasetSummaryElement = document.getElementById(
    "sam2-yolo-dataset-summary",
  );
  const yoloClassTabTemplate = document.getElementById(
    "sam2-yolo-class-template",
  );
  const yoloFileTabTemplate = document.getElementById(
    "sam2-yolo-file-template",
  );
  const yoloClassEmptyTemplate = document.getElementById(
  ...
  if (yoloTrainButtonGroup) {
    ...
    buttonColumn?.parentElement
      ?.querySelectorAll(':scope > [class*="col-md-"]')
      .forEach((column) => {
        column.classList.remove("col-md-2", "col-md-5");
        column.classList.add("col-md-4");
      });
    buttonColumn?.classList.add("align-self-center");
    [yoloTrainStartButton, yoloRetrainStartButton, yoloTrainStopButton].forEach(
      (button) => {
        button?.classList.add("text-nowrap");
      },
    );
  }
  const yoloTrainProgressElement = document.getElementById(
    "sam2-yolo-train-progress",
  );
  const yoloTrainProgressTextElement = document.getElementById(
    "sam2-yolo-train-progress-text",
  );
  ...
  function setYoloOverlayPolygonLoadingToast(visible, hostElement) {
    ...
    if (!yoloOverlayPolygonToastElement) {
      ...
      yoloOverlayPolygonToastElement.innerHTML =
        '<div class="toast-body text-start text-nowrap pe-2"><span class="spinner-border spinner-border-sm me-2" aria-hidden="true"></span>검출 폴리곤 데이터를 오버레이에 표시하는 중...</div>';
  ...
  function setUploadProgress(percent) {
    ...
    if (uploadProgressBarElement) {
      ...
      uploadProgressBarElement.setAttribute(
        "aria-valuenow",
        String(normalized),
      );
    }
  ...
  function getPromptFrame() {
    const maximumFrame =
      inputVideoFrameCount > 0 ? inputVideoFrameCount : Number.MAX_SAFE_INTEGER;
    return clamp(
  ...
  function restorePendingPromptFrame() {
    ...
    const targetFrame = clamp(
      Math.trunc(toNumber(pendingPromptFrame, 1)),
      1,
      maximumFrame,
    );
  ...
  function updateInputFrameCounter(mediaTime) {
    ...
    const frameTime = Number.isFinite(mediaTime)
      ? mediaTime
      : inputVideoElement.currentTime;
    ...
    if (
      promptFrameInput &&
      pendingPromptFrame === null &&
      document.activeElement !== promptFrameInput
    ) {
  ...
  async function loadInputVideoMetadata(fileName) {
    ...
    try {
      ...
      const response = await fetch(
        `${apiBase}/fast/sam2/video_metadata?file_name=${encodeURIComponent(fileName)}`,
        {
          cache: "no-store",
        },
      );
      ...
      inputVideoFrameCount = Math.max(
        0,
        Math.trunc(Number(metadata.frame_count) || 0),
      );
      updateInputFrameCounter();
      updateOutputPlaybackControls();
  ...
  function updateMultimaskOutputText() {
    if (multimaskOutputText) {
      multimaskOutputText.textContent =
        multimaskOutputCheckbox && multimaskOutputCheckbox.checked
          ? "Multi mask"
          : "Single mask";
    }
  }
  ...
  function updateDetectionControlState() {
    ...
    [
      foregroundPointModeButton,
      backgroundPointModeButton,
      bboxModeButton,
      foregroundPointClearButton,
      backgroundPointClearButton,
      bboxClearButton,
      optionsSaveButton,
      optionsResetButton,
      multimaskOutputCheckbox,
      maskInputCheckbox,
      claheCheckbox,
      maskRatioInput,
    ].forEach((control) => {
    ...
    if (promptFrameInput) {
      promptFrameInput.disabled =
        !enabled || inputVideoFps <= 0 || inputVideoFrameCount <= 0;
    }
  ...
  function loadSelectedVideo() {
    try {
      return String(
        localStorage.getItem(STORAGE_SELECTED_VIDEO_KEY) || "",
      ).trim();
    } catch (_ignore) {
      return "";
    }
  ...
  async function loadVideoOptions(fileName) {
    ...
    try {
      const apiBase = await resolveApiBase();
      const response = await fetch(
        `${apiBase}/fast/sam2/video_options?file_name=${encodeURIComponent(value)}`,
      );
      if (!response.ok) {
        return false;
      }
      ...
      pendingPromptFrame = Math.max(
        1,
        Math.trunc(toNumber(options.prompt_frame, 1)),
      );
      restorePendingPromptFrame();

      const points = Array.isArray(options.points) ? options.points : [];
      const labels = Array.isArray(options.point_labels)
  ...
  function updateOutputPlaybackControls(mediaTime) {
    const hasMedia =
      Boolean(outputVideoElement.currentSrc) &&
      outputVideoElement.readyState >= HTMLMediaElement.HAVE_METADATA;
    const isPlaying =
      hasMedia && !outputVideoElement.paused && !outputVideoElement.ended;
    ...
    const canSeekByFrame =
      hasMedia && outputVideoFps > 0 && outputVideoFrameCount > 0;
    ...
    outputFrameSliderElement.max = String(
      canSeekByFrame ? outputVideoFrameCount : 1,
    );
    outputFrameSpinnerElement.disabled = !canSeekByFrame;
    outputFrameSpinnerElement.min = "1";
    outputFrameSpinnerElement.max = String(
      canSeekByFrame ? outputVideoFrameCount : 1,
    );
    if (!canSeekByFrame) {
    ...
    const frameTime = Number.isFinite(mediaTime)
      ? mediaTime
      : outputVideoElement.currentTime;
    const currentFrame = clamp(
  ...
  function seekOutputFrame(frameValue) {
    ...
    outputVideoElement.currentTime = Math.min(
      Number.isFinite(outputVideoElement.duration)
        ? outputVideoElement.duration
  ...
  function getActiveOutputFrameInput() {
    ...
    if (
      document.activeElement === outputFrameSpinnerElement ||
      document.activeElement === outputFrameSliderElement
    ) {
  ...
  function initializeOutputVideoControls() {
    ...
    [
      ...
      outputVideoElement.addEventListener(eventName, () =>
        updateOutputPlaybackControls(),
      );
    });
    outputPlayToggleButton?.addEventListener("click", toggleOutputPlayback);
    outputFrameSliderElement?.addEventListener(
      "keydown",
      handleOutputFrameKeyboard,
    );
    outputFrameSpinnerElement?.addEventListener(
      "keydown",
      handleOutputFrameKeyboard,
    );
    outputFrameSliderElement?.addEventListener("input", () =>
      seekOutputFrame(outputFrameSliderElement.value),
    );
    outputFrameSpinnerElement?.addEventListener("input", () =>
      seekOutputFrame(outputFrameSpinnerElement.value),
    );
    updateOutputPlaybackControls();
  }
  ...
  function initializeYoloOutputTab() {
    ...
    yoloClassTabContentElement = document.getElementById(
      "sam2-yolo-class-tab-content",
    );
    updateYoloClassTabs();
    ...
    yoloTrainStartButton?.addEventListener("click", () =>
      startYoloTraining(false),
    );
    yoloRetrainStartButton?.addEventListener("click", () =>
      startYoloTraining(true),
    );
  ...
  function renderYoloTrainingProgress(
    progress,
    message,
    status,
    elapsedSeconds,
    ...
    if (yoloTrainProgressElement) {
      ...
      yoloTrainProgressElement
        .closest(".progress")
        ?.setAttribute("aria-valuenow", String(normalizedProgress));
      yoloTrainProgressElement.classList.toggle(
  ...
  async function pollYoloTrainingStatus() {
    ...
    try {
      const apiBase = await resolveApiBase();
      const response = await fetch(
        `${apiBase}/fast/sam2/yolo_training_status/${encodeURIComponent(yoloTrainingJobId)}`,
        { cache: "no-store" },
      );
      if (!response.ok) {
      ...
      const copiedWeightPath = String(
        (job.result && job.result.best_model_path) || "",
      ).trim();
      const message =
        status === "completed" && copiedWeightPath
  ...
  async function recoverActiveYoloTraining() {
    ...
    try {
      const apiBase = await resolveApiBase();
      const response = await fetch(
        `${apiBase}/fast/sam2/yolo_training_status`,
        { cache: "no-store" },
      );
      if (!response.ok) {
  ...
  async function updateYoloTrainingOverview() {
    ...
    try {
      const apiBase = await resolveApiBase();
      const response = await fetch(
        `${apiBase}/fast/sam2/yolo_dataset_summary`,
        { cache: "no-store" },
      );
      if (!response.ok) {
  ...
  function updateOutputDownloadState() {
    ...
    if (yoloConvertButton) {
      yoloConvertButton.disabled =
        !yoloConversionAvailable || !yoloInputFileName;
    }
  }
  ...
  async function convertYoloDataset() {
    ...
    const className =
      extractYoloClassName(yoloInputFileName) ||
      basename(yoloInputFileName).replace(/\.[^.]+$/, "");
    if (yoloConvertButton) {
    ...
    try {
      ...
      if (conversionCompleted) {
        ...
        const convertedItem = uploadedHistory.find(
          (item) => basename(item && item.name) === convertedInputName,
        );
        if (convertedItem) {
      ...
      setStatus(
        conversionCompleted
          ? `${className} 클래스 YOLO 변환 완료`
          : "변환할 학습 데이터가 없습니다.",
        conversionCompleted ? "success" : "warning",
      );
    } catch (error) {
      yoloConversionAvailable = true;
      updateOutputDownloadState();
      setStatus(
        error && error.message ? error.message : "YOLO 변환에 실패했습니다.",
        "danger",
      );
    }
  }
  ...
  function renderPointUi() {
    ...
    renderPointList(
      positivePointListElement,
      positivePoints,
      "sam2-point-chip-positive",
    );
    renderPointMarkers();
  }
  ...
  function clearPointsByLabel(label) {
    ...
    positivePoints = positivePoints.filter(
      (point) => (Number(point && point.label) === 0 ? 0 : 1) !== pointLabel,
    );
    renderPointUi();
  ...
  function showPointContextMenu(event, pointIndex) {
    ...
    deleteButton.addEventListener("click", () => {
      ...
      if (selectedPoint) {
        ...
        renderPointUi();
        setStatus(
          `${Number(selectedPoint.label) === 0 ? "Background" : "Foreground"} Point를 삭제했습니다.`,
          "secondary",
        );
      }
      hidePointContextMenu();
  ...
  function addPointByClick(event) {
    ...
    if (positivePoints.length >= MAX_POINT_COUNT) {
      setStatus(
        `Point는 최대 ${MAX_POINT_COUNT}개까지 입력할 수 있습니다.`,
        "warning",
      );
      return;
    }
    ...
    renderPointUi();
    setStatus(
      `${point.label === 1 ? "Foreground" : "Background"} Point가 추가되었습니다.`,
      "secondary",
    );
  }
  ...
  function clampPointToBoundingBox(point) {
    ...
    return {
      x: clamp(
        point.x,
        toNumber(box.x, 0),
        toNumber(box.x, 0) + toNumber(box.w, 0),
      ),
      y: clamp(
        point.y,
        toNumber(box.y, 0),
        toNumber(box.y, 0) + toNumber(box.h, 0),
      ),
    };
  }
  ...
  function renderBboxControlPoints() {
    ...
    bboxCaptureLayerElement
      .querySelectorAll(".sam2-bbox-control-point")
      .forEach((node) => {
        node.remove();
      });
  ...
  function buildPointLabelsQuery() {
    const labels = Array.isArray(positivePoints)
      ? positivePoints.map((point) =>
          Number(point && point.label) === 0 ? 0 : 1,
        )
      : [];
  ...
  function updateMaskInputText() {
    if (maskInputText) {
      maskInputText.textContent =
        maskInputCheckbox && maskInputCheckbox.checked ? "On" : "Off";
    }
  }
  ...
  function updateClaheText() {
    if (claheText) {
      claheText.textContent =
        claheCheckbox && claheCheckbox.checked ? "On" : "Off";
    }
  }
  ...
  async function saveVideoOptions(fileName) {
    ...
    const apiBase = await resolveApiBase();
    const url = `${apiBase}/fast/sam2/video_options?file_name=${encodeURIComponent(value)}&model_name=auto${buildPromptFrameQuery()}${buildBboxQuery()}${buildPointsQuery()}${buildPointLabelsQuery()}${buildMultimaskOutputQuery()}${buildMaskInputQuery()}${buildClaheQuery()}${buildReferenceScoreQuery()}${buildMaskAreaRatioQuery()}`;
  ...
  async function waitForSegmentResult(apiBase, response) {
    ...
    while (true) {
      ...
      const progress = Math.max(
        0,
        Math.min(99, Number(statusResult.progress) || 0),
      );
  ...
  async function previewSelectedVideoFirstFrame(showInputTab, inputPath) {
    ...
    const apiBase = await resolveApiBase();
    const inputPathUrl =
      String(inputPath || "").trim() || `/fast/image/${selectedServerFileName}`;
    debugSam2("원본 영상 URL 확인", { apiBase, inputPathUrl });
    try {
      const inputUrl = await resolvePlayableVideoUrl(
        apiBase,
        inputPathUrl,
        true,
      );
  ...
  function sortUploadedHistoryByName(items) {
    return items.sort((left, right) =>
      String((left && left.name) || "").localeCompare(
  ...
  async function deleteYoloDataset(item, deleteButton) {
    ...
    if (
      !window.confirm(`${inputStem}의 YOLO 학습 데이터를 삭제하시겠습니까?`)
    ) {
      return;
    }
    ...
    try {
      ...
      updateYoloTrainingOverview();
      setStatus(
        `${inputStem} 학습 데이터 ${Number(result.deleted_count || 0)}개 삭제 완료`,
        "success",
      );
    } catch (error) {
  ...
  function initializeYoloFrameViewer(item, fileTabPane) {
    const loadingElement = fileTabPane.querySelector(
      '[data-role="frame-loading"]',
    );
    const viewerElement = fileTabPane.querySelector(
    ...
    const imageTabButton = fileTabPane.querySelector(
      '[data-role="frame-image-tab"]',
    );
    const maskTabButton = fileTabPane.querySelector(
      '[data-role="frame-mask-tab"]',
    );
    const overlayTabButton = fileTabPane.querySelector(
      '[data-role="frame-overlay-tab"]',
    );
    const segTabButton = fileTabPane.querySelector(
      '[data-role="frame-seg-tab"]',
    );
    const imageTabPane = fileTabPane.querySelector(
      '[data-role="frame-image-pane"]',
    );
    const maskTabPane = fileTabPane.querySelector(
      '[data-role="frame-mask-pane"]',
    );
    const overlayTabPane = fileTabPane.querySelector(
      '[data-role="frame-overlay-pane"]',
    );
    const segTabPane = fileTabPane.querySelector(
      '[data-role="frame-seg-pane"]',
    );
    ...
    const overlayImageElement = fileTabPane.querySelector(
      '[data-role="frame-overlay-image"]',
    );
    const overlayMaskElement = fileTabPane.querySelector(
      '[data-role="frame-overlay-mask"]',
    );
    const classIdElement = fileTabPane.querySelector(
      '[data-role="frame-class-id"]',
    );
    const classNameElement = fileTabPane.querySelector(
      '[data-role="frame-class-name"]',
    );
  ...
  function updateYoloClassTabs(preferredClassName) {
    if (
      !yoloClassTabsElement ||
      !yoloClassTabContentElement ||
      !yoloClassTabTemplate ||
      !yoloFileTabTemplate ||
      !yoloClassEmptyTemplate
    ) {
      return;
    }

    const classNames = getYoloClassNames();
    const currentActiveClassName =
      yoloClassTabsElement.querySelector(".nav-link.active")?.dataset
        .className || "";
    ...
    if (classNames.length === 0) {
      yoloClassTabContentElement.appendChild(
        yoloClassEmptyTemplate.content.cloneNode(true),
      );
      return;
    }
    ...
    classNames.forEach((className, index) => {
      ...
      const classVideos = uploadedHistory.filter(
        (item) =>
          item &&
      ...
      const fileTabContentElement = fragment.querySelector(
        '[data-role="file-tab-content"]',
      );
      if (
        !tabItem ||
        !tabButton ||
      ...
      tabButton.textContent = `${className}(${index}) ${classVideos.length}개`;
      ...
      const activeFileIndex = Math.max(
        0,
        classVideos.findIndex(
          (item) => basename(item && item.name) === activeInputName,
        ),
      );
      if (classVideos.length === 0) {
        fileTabContentElement.appendChild(
          yoloClassEmptyTemplate.content.cloneNode(true),
        );
      }
      classVideos.forEach((item, fileIndex) => {
        ...
        const fileTabItem = fileFragment.querySelector(
          '[data-role="file-tab-item"]',
        );
        const fileTabButton = fileFragment.querySelector(
          '[data-role="file-tab-button"]',
        );
        const fileTabTitle = fileFragment.querySelector(
          '[data-role="file-tab-title"]',
        );
        const deleteButton = fileFragment.querySelector(
          '[data-role="dataset-delete"]',
        );
        const fileTabPane = fileFragment.querySelector(
          '[data-role="file-tab-pane"]',
        );
        if (
          !fileTabItem ||
          !fileTabButton ||
  ...
  function renderUploadedHistory() {
    ...
    uploadedListElement
      .querySelectorAll('li[data-uploaded-history-item="true"]')
      .forEach((item) => {
        item.remove();
      });
    uploadedListElement
      .querySelectorAll("#sam2-uploaded-empty")
      .forEach((item) => {
        item.remove();
      });
    ...
    for (const item of uploadedHistory) {
      ...
      if (
        item.serverFileName &&
        item.serverFileName === highlightedServerFileName
      ) {
      ...
      row.addEventListener("click", async () => {
        ...
        try {
          await previewSelectedVideoFirstFrame(
            false,
            item.playableUrl || item.inputUrl,
          );
        } catch (error) {
          const message =
            error && error.message ? error.message : "원본 영상 로드 실패";
  ...
  async function deleteUploadedVideo(item) {
    ...
    try {
      let response = await fetch(
        `/fast/sam2/uploaded_video?file_name=${encodeURIComponent(item.serverFileName)}`,
        {
          method: "DELETE",
        },
      );
      if (!response.ok) {
        ...
        response = await fetch(
          `${apiBase}/fast/sam2/uploaded_video?file_name=${encodeURIComponent(item.serverFileName)}`,
          {
            method: "DELETE",
          },
        );
      ...
      uploadedHistory = uploadedHistory.filter(
        (historyItem) => historyItem.serverFileName !== item.serverFileName,
      );
  ...
  document.addEventListener("click", (event) => {
    if (
      uploadedContextMenuElement &&
      !uploadedContextMenuElement.contains(event.target)
    ) {
      hideUploadedContextMenu();
    }
  });
  ...
  function setUploadedListLoading(isLoading, message) {
    ...
    uploadedListLoadingMessage = String(
      message || uploadedListLoadingMessage || "동영상 목록을 불러오는 중...",
    );
  ...
  async function finishUploadedListLoading(doneMessage) {
    ...
    if (
      completionText &&
      uploadedLoadingElement &&
      uploadedLoadingTextElement
    ) {
  ...
  function addUploadedHistoryItem(
    fileNameHint,
    inputFilePath,
    thumbnailSource,
  ) {
    const displayName =
      basename(fileNameHint) || basename(inputFilePath) || "unknown_video";
    const now = new Date();
    const hh = String(now.getHours()).padStart(2, "0");
    const mm = String(now.getMinutes()).padStart(2, "0");
    const ss = String(now.getSeconds()).padStart(2, "0");

    const duplicateIndex = uploadedHistory.findIndex(
  ...
  async function loadUploadedHistoryFromServer() {
    ...
    try {
      ...
      if (!response.ok) {
        apiBase = await resolveApiBase();
        response = await fetch(
          `${apiBase}/fast/sam2/uploaded_videos?limit=500`,
          {
            method: "GET",
            cache: "no-store",
          },
      ...
      const matchedSelected = mapped.find(
        (item) => item.serverFileName === savedSelectedVideo,
      );
      if (matchedSelected) {
        if (!selectedServerFileName) {
          showInputSourceTab("sam2-uploaded-source-tab");
          ...
          try {
            await previewSelectedVideoFirstFrame(
              false,
              matchedSelected.playableUrl || matchedSelected.inputUrl,
            );
          } catch (error) {
            const message =
              error && error.message ? error.message : "원본 영상 로드 실패";
        ...
        saveSelectedVideo("");
        setStatus(
          `업로드 목록 ${mapped.length}건을 불러왔습니다.`,
          "secondary",
        );
      } else {
        setStatus(
      ...
      if (requestSeq === uploadedListLatestRequestSeq) {
        const message =
          "업로드 목록을 불러오지 못했습니다. API 연결 상태를 확인하세요.";
        setStatus(message, "warning");
        window.alert(message);
      }
    } finally {
  ...
  async function loadExistingOutputVideo(
    outputUrl,
    inputFileName,
    conversionAvailable,
  ) {
    ...
    try {
      ...
      if (!metadataResponse.ok) {
        throw new Error(
          `검출 영상 메타데이터 조회 실패 (${metadataResponse.status})`,
        );
      }
      const metadata = await metadataResponse.json();
      outputVideoFps = Math.max(0, Number(metadata.fps) || 0);
      outputVideoFrameCount = Math.max(
        0,
        Math.trunc(Number(metadata.frame_count) || 0),
      );
      updateOutputPlaybackControls();
  ...
  async function assignVideoSource(videoElement, sourceUrl, objectUrlKey) {
    ...
    await new Promise((resolve, reject) => {
      ...
      const onError = () => {
        ...
        const code =
          mediaError && mediaError.code ? mediaError.code : "unknown";
        cleanup();
  ...
  async function handleChosenFile(file) {
    ...
    if (maxUploadBytes > 0 && Number(file.size || 0) > maxUploadBytes) {
      ...
      setStatus(
        `파일 용량이 너무 큽니다. 최대 크기는 ${maxText} 입니다.`,
        "warning",
      );
      return;
    }
    ...
    if (isUploadingImmediately) {
      setStatus(
        "다른 동영상 업로드가 진행 중입니다. 잠시 후 다시 시도하세요.",
        "warning",
      );
      return;
    }
    ...
    try {
      ...
      const uploadedPath =
        extractFastImagePath(uploadResult.input_url) ||
        String(uploadResult.file_name || "").trim();
      selectedFile = null;
      if (fileInput) {
        fileInput.value = "";
      }
      ...
      try {
        await previewSelectedVideoFirstFrame(
          false,
          uploadResult.playable_url || uploadResult.input_url,
        );
      } catch (_ignore) {
      ...
      setStatus(
        `업로드 오류: ${message} (검출 시작 시 업로드 재시도)`,
        "danger",
      );
    } finally {
  ...
  async function runSam2Segment() {
    ...
    if (!file && !selectedServerFileName && highlightedServerFileName) {
      const highlightedExists = uploadedHistory.some(
        (item) => item.serverFileName === highlightedServerFileName,
      );
      if (highlightedExists) {
        selectedServerFileName = highlightedServerFileName;
        saveSelectedVideo(selectedServerFileName);
        setStatus(
          "하이라이트된 동영상을 자동 선택하여 검출을 시작합니다.",
          "secondary",
        );
      }
    }
    ...
    try {
      ...
      if (file) {
        const formData = new FormData();
        formData.append("file", file);
        const url = `${apiBase}/fast/sam2/segment_video_upload?${bboxQuery.slice(1)}${pointsQuery}${pointLabelsQuery}${promptFrameQuery}${buildMultimaskOutputQuery()}${buildMaskInputQuery()}${buildClaheQuery()}${buildReferenceScoreQuery()}${buildMaskAreaRatioQuery()}`;
        response = await fetch(url, {
          method: "POST",
          body: formData,
        });
      } else {
        const url = `${apiBase}/fast/sam2/segment_saved_video?file_name=${encodeURIComponent(selectedServerFileName)}${bboxQuery}${pointsQuery}${pointLabelsQuery}${promptFrameQuery}${buildMultimaskOutputQuery()}${buildMaskInputQuery()}${buildClaheQuery()}${buildReferenceScoreQuery()}${buildMaskAreaRatioQuery()}`;
        response = await fetch(url, {
          method: "POST",
        });
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 504) {
          errorMessage =
            "요청 실패 (504): 처리 시간이 초과되었습니다. 짧은 영상으로 시도하거나 서버를 재시작 후 다시 실행하세요.";
        }
        try {
          const errorBody = await response.json();
          if (errorBody && errorBody.detail) {
      ...
      if (file) {
        const relativeServerPath =
          extractFastImagePath(result.input_url) ||
          String(result.input_file || "").trim();
        selectedServerFileName = String(
          relativeServerPath || selectedServerFileName,
        );
        highlightedServerFileName = selectedServerFileName;
        saveSelectedVideo(selectedServerFileName);
        await loadUploadedHistoryFromServer();
      ...
      const outputUrl = await resolvePlayableVideoUrl(
        apiBase,
        result.output_url,
        true,
      );
      yoloInputFileName =
        extractFastImagePath(result.input_url) || selectedServerFileName || "";
      ...
      outputVideoFrameCount = Math.max(
        0,
        Math.trunc(Number(result.input_total_frames) || 0),
      );
      ...
      if (file) {
        try {
          ...
          const uploadResponse = await fetch(
            `${apiBase}/fast/sam2/upload_video`,
            {
              method: "POST",
  ...
  document
    .querySelectorAll('#sam2-input-source-tabs [data-bs-toggle="tab"]')
    .forEach((tabButton) => {
  ...
  document
    .querySelectorAll('#sam2-video-tabs [data-bs-toggle="tab"]')
    .forEach((tabButton) => {
  ...
  function setPointMode(mode) {
    ...
    backgroundPointModeButton?.classList.toggle(
      "btn-outline-primary",
      isForeground,
    );
    ...
    setStatus(
      `${isForeground ? "전경" : "배경"} Point 설정 모드입니다.`,
      "secondary",
    );
  }
  ...
  if (optionsResetButton) {
    optionsResetButton.addEventListener("click", async () => {
      ...
      setStatus(
        restored
          ? "변경사항을 취소하고 서버에 저장된 검출 설정을 복원했습니다."
          : "서버 검출 설정을 불러오지 못했습니다.",
  ...
  if (bboxCaptureLayerElement) {
    ...
    bboxCaptureLayerElement.addEventListener(
      "contextmenu",
      selectPointByRightClick,
    );
    bboxCaptureLayerElement.addEventListener(
      "mousemove",
      updatePointInputCursor,
    );
  ...
  if (multimaskOutputCheckbox) {
    multimaskOutputCheckbox.addEventListener(
      "change",
      updateMultimaskOutputText,
    );
    updateMultimaskOutputText();
